...
Stone Set is a private two-user muscle-growth training system with independently managed routines, exercise guidance, and a shared normalized rank economy.
...
- user-owned versioned descriptions, muscle targeting, images, and YouTube references;
- private exercise images in Supabase Storage;
- official YouTube embedded playback in Android;
...
- no RR or XP for unscheduled extra volume or viewing guidance.
...
- Supabase Auth, Postgres, and private Storage with RLS;
- SQLite local workout drafts and active-session guidance cache;
- online session start and server-authoritative finalization;
- YouTube IFrame Player API in an Android WebView;
- Vercel static dashboard hosting;
- separate local, staging, and production Supabase environments;
- Supabase Pro production daily database backups plus encrypted database and Storage exports.
...
The task creates scaffolding and quality gates only. It does not implement authentication, routines, exercise guidance, media, YouTube playback, workouts, rank behavior, product schema, or deployment.
