...
void main() {
  test('restores a verified active session and caches its bootstrap', () async {
    ...
    expect(
      store.identityByOwner[syntheticUserId]?.profile.userId,
      syntheticUserId,
    );
  });
  ...
  test('first launch without a persisted session remains signed out', () async {
    final repository = FakeIdentityRepository(
      refreshFailure: const IdentityFailure(
  ...
  test(
    'matching verified cache renders before any network refresh is attempted',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(
      ...
      final state = await container.read(
        mobileSessionControllerProvider.future,
      );
  ...
  test(
    'transport failure without matching cache does not expose protected content',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(
        initialSession: session,
  ...
  test(
    'cache for the wrong owner is rejected during offline restoration',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(
        initialSession: session,
  ...
  test(
    'password-change-required cache cannot authorize offline protected content',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(
        initialSession: session,
        refreshFailure: const IdentityFailure(
          IdentityErrorCode.serverUnavailable,
        ),
      );
      final store = FakeMobileSnapshotStore()
        ..identityByOwner[syntheticUserId] = syntheticBootstrap(
  ...
  test(
    'transient auth stream failures keep a verified cached shell exposed',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(initialSession: session);
  ...
  test(
    'password recovery events cannot become normal authenticated sessions',
    () async {
      const session = IdentitySession(userId: syntheticUserId, expiresAt: null);
      final repository = FakeIdentityRepository(initialSession: session);
      final quarantine = _RecordingQuarantine();
      ...
      repository.emit(
        const IdentityAuthEvent(IdentityAuthEventType.passwordRecovery),
      );
      await Future<void>.delayed(Duration.zero);
  ...
  test(
    'rejects a profile that does not match the authenticated user',
    () async {
...
ProviderContainer _container(
  ...
  return ProviderContainer(
    overrides: [
      ...
      mobileSnapshotStoreProvider.overrideWithValue(
        store ?? FakeMobileSnapshotStore(),
      ),
