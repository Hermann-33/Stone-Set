...
void main() {
  test('identity bootstrap round-trips without changing owner eligibility', () {
    ...
    expect(
      decoded.profile.normalizedUsername,
      source.profile.normalizedUsername,
    );
    ...
    expect(
      () => validateIdentityOwner(syntheticUserId, decoded),
      returnsNormally,
    );
  });
  ...
  test('Week round-trip preserves materialized calendar dates and owner', () {
    ...
    expect(
      decoded.week?.items.first.currentDate,
      source.week?.items.first.currentDate,
    );
  ...
  test(
    'Progress round-trip preserves authoritative rank and finalized history',
    () {
      final decoded = decodeProgressSnapshot(
        encodeProgressSnapshot(defaultProgressSnapshot),
      );

      expect(decoded.account.userId, syntheticUserId);
      expect(
        decoded.account.rrBalance,
        defaultProgressSnapshot.account.rrBalance,
      );
      ...
      expect(
        decoded.workouts.single.resultId,
        defaultProgressSnapshot.workouts.single.resultId,
      );
      expect(
        () => validateProgressOwner(syntheticUserId, decoded),
        returnsNormally,
      );
    },
  );
  ...
  test('owner validation rejects cross-account cached payloads', () {
    ...
    expect(
      () => validateWeekOwner(otherOwner, standardWeek()),
      throwsFormatException,
    );
    expect(
